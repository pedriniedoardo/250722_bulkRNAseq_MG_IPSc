# AIM ---------------------------------------------------------------------
# the aim of the script is to read in the raw table of counts and build the dds object for the LRT analysis on the cytokine samples
...
LUT_samples <- read_csv("../../data/LUT_sample.csv") %>%
  # reduce the samples to only control and cytokine
  dplyr::filter(treat %in% c("untreated","cytokine"))
...
# batch <- coldata$batch
# treat <- factor(coldata$treat,levels = c("untreated","cytokine"))
exposure <- factor(coldata$exposure, levels = c("0","6","24"))

# build the design
# design <- model.matrix(~ clone + batch + gender + treat)
design <- model.matrix(~ clone + exposure)
colnames(design)[1] <- c("intercept")

saveRDS(design,file = "../../out/object/design_cytokine_LRT.rds")
...
saveRDS(dds,file = "../../out/object/dds_cytokine_LRT.rds")
